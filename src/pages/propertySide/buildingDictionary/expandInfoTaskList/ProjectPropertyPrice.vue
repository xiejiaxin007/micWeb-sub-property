            ...
            <el-table
                ...
                <el-table-column
                    ...
                    <template slot-scope="scope">
                        <jl-number-input
                            v-model="scope.row.property_fee.field_value"
                            placeholder="请输入物业费"
                            minVal="0.01"
                            maxVal="9999.99"
                            size=""
                ...
                <el-table-column
                    align="center"
                    v-if="tableItemShows['property_fee'] && tableColStatus['property_fee'] == '1'">
                    <template
                        slot="header">
                        <info-source-dialog 
                            v-model="projectPropertyYearSel.value"
                            @change="(val, otherVal) => changeAllSource('property_fee', val, otherVal)"
                            :other-val.sync="projectPropertyYearSel.otherVal"
                            :origin-arr="getConfigList.qd_source_list" />
                    </template>
                    <template
                        slot-scope="scope"
                        v-if="scope.row.property_fee">
                        <info-source-dialog
                            :other-val.sync="scope.row.property_fee.belong_other"
                            :origin-arr="getConfigList.qd_source_list"
                            v-model="scope.row.property_fee.belong_role" />
                    </template>
                </el-table-column>
                <el-table-column
                    label="关联楼栋"
...
export default {
    ...
    methods: {
        ...
        pushDataToParents() {
            return {
                originData: this.propertyPriceInfoCopy,
                newData: this.propertyPriceInfo,
        ...
        notSelectCompany() {
            ...
            let hashList = {};
            let flag = true;
            this.propertyPriceInfo.forEach(item => {
                arrList.push(item.property_fee.field_value);
                if (item.property_fee.field_value == '') {
                    flag = false;
                }
            })
            if (!flag) {
        ...
        copyTbaleData() {
            ...
            if (this.tableDataInfo.project_property_fee_info.data.length <= 0) {
                ...
                this.propertyPriceInfo.push(JSON.parse(JSON.stringify(this.initStructureData.project_property_fee_info.data)));
                this.propertyPriceInfoCopy.push(this.initStructureDataCopy.project_property_fee_info.data);
            } else {
                this.propertyPriceInfo = this.tableDataInfo.project_property_fee_info.data;
                this.propertyPriceInfoCopy = this.tableDataInfoCopy.project_property_fee_info.data;
            }
            // 初始化的时候去重校验
            this.propertyPriceInfo.forEach(item => {
        ...
        delTableLine(selVal, index) {
            this.$confirm('一旦删除则无法恢复确认是否删除！', '提示', {
                ...
                type: 'warning'
                    ...
                    if (selVal === '') {
                        // 删除编辑中数组的数据
                        this.propertyPriceInfo.splice(index, 1);
                        this.initHouseInfo(this.propertyPriceInfo);
                        // 删除源数组的数据
                        this.propertyPriceInfoCopy.splice(index, 1);
                        this.$message({
                            type: 'success',
                            message: '删除成功!'
                        });
                    ...
                    try {
                        ...
                        let result = await propertyApi.delPropertyPriceData(params);
                        let { data } = result;
                        if (data.code === 0) {
                            // 删除编辑中数组的数据
                            this.propertyPriceInfo.splice(index, 1);
